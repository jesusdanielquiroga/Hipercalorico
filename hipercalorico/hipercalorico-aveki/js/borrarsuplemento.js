function mostrarEliminar(id, infsup){
    $("#editar").hide();
    $("#listado").hide();
    $("#nuevoRegistro").hide();
    $("#titleIdDelete").html("Desea eliminar el suplemento con la referencia "+ id + " ?...");    
    $("#idDelete").val(id);
    $("#supplementDelete").html(infsup);
    $("#eliminar").show(1000);
}
/*
...
function borrarRegistro(llaveRegistro) {
    ...
    $.ajax({
        ...
        error: function (xhr, status) {
            $("#mensajes").html("El suplemento que intenta eliminar tiene información relacionada, por favor verifique...");
            $("#mensajes").show(1000);
        }
    });
}