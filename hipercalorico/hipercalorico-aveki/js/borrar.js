function mostrarEliminar(iduser,infuser){
    $("#editar").hide();
    $("#listado").hide();
    $("#nuevoRegistro").hide();
    $("#titleIdDelete").html("Desea eliminar el usuario con la identificación: " + iduser + " ?...");    
    $("#idDelete").val(iduser);
    $("#userDelete").html(infuser);
    $("#eliminar").show(1000);
}
/*
...
function borrarRegistro(llaveRegistro) {
    ...
    $.ajax({
        ...
        error: function (xhr, status) {
            $("#mensajes").html("El usuario que intenta eliminar tiene información relacionada, por favor verifique...");
            $("#mensajes").show(1000);
        }
    });
}